...
bool Laberinto::ResolverLaberinto(std::ofstream& output_file) {
  ...
  while (!open_list.empty()) {
    ...
    open_list.pop();
    // contaje de nodos inspeccionados (extraídos para procesar)
    ...
    for (const auto& direccion : direcciones) {
      ...
      if (nueva_fila >= 0 && nueva_fila < num_filas && nueva_columna >= 0 &&
        ...
        // Ahora: usar el COSTE_DIAGONAL para el cálculo de g cuando el
...
bool Laberinto::ResolverLaberintoDinamico(std::ofstream& output_file) {
  ...
  const int COSTE_DIAGONAL = 7;
  const int FACTOR_HEURISTICO = 3;
  const double PROB_CAMBIO_POR_CASILLA = 0.30;  // probabilidad de cambio después de cada inspección
  ...
  int max_muros_permitidos = static_cast<int>(PORCENTAJE_MAXIMO_MUROS * total_casillas);
  ...
  // Matriz que indica las casillas por las que EL AGENTE ya ha pasado (no se modificarán)
  std::vector<std::vector<bool>> visitado_por_agente(num_filas, std::vector<bool>(num_columnas, false));
  ...
  while (!open_list.empty()) {
    ...
    if (laberinto_[nodo_actual.posicion.first][nodo_actual.posicion.second] == 1) {
      continue;
    }

    // Si este nodo es el siguiente movimiento del agente (padre == posicion_agente),
    // el agente lo "elige" y se mueve: en ese momento marcamos como visitado y imprimimos.
    bool agente_se_mueve_a_este_nodo = (nodo_actual.posicion != posicion_agente &&
                                        nodo_actual.padre == posicion_agente);

    if (agente_se_mueve_a_este_nodo) {
      // actualizar posición del agente y marcar como visitada (no se modificarán en el futuro)
      ...
      // impresión: solo cuando el agente elige y se mueve
      {
        ...
        // reconstruir camino recorrido por el agente hasta su nueva posición usando 'padres'
        std::pair<int,int> p = posicion_agente;
        while (!(p.first == -1 && p.second == -1) && !(p == inicio_)) {
          if (temp_matriz[p.first][p.second] == 0) temp_matriz[p.first][p.second] = 2;
        ...
        output_file << "Iteración (movimiento) " << iteracion
                    << " - Agente se mueve a: (" << nodo_actual.posicion.first << ", " << nodo_actual.posicion.second << ")"
                    << "  g=" << nodo_actual.g << std::endl;
        output_file << "Camino recorrido por el agente (marcado con 2):" << std::endl;
    ...
    // y NUNCA sobre celdas visitadas por el agente (incluye inicio y posiciones ya recorridas)
    if (iteracion > 1) {
      for (int i = 0; i < num_filas; ++i) {
        for (int j = 0; j < num_columnas; ++j) {
          if ((i == inicio_.first && j == inicio_.second) || (i == fin_.first && j == fin_.second)) continue;
          if (visitado_por_agente[i][j]) continue; // no cambiar celdas ya recorridas por el agente
      ...
      // Asegurar que el número de muros no supere el 25%: eliminar muros NO visitados por el agente
    ...
    for (const auto& direccion : direcciones) {
      ...
      if (nueva_fila >= 0 && nueva_fila < num_filas &&
        ...
        int coste_movimiento_para_g = es_movimiento_ortogonal ? COSTE_HORIZONTAL_VERTICAL : COSTE_DIAGONAL;
        int nuevo_g = nodo_actual.g + coste_movimiento_para_g;
  ...
  if (encontrado) {
    ...
    MostrarLaberinto(output_file, coste_total, nodos_generados, nodos_inspeccionados);
    return true;
  } else {
    std::cout << "No se encontró solución para el laberinto dinámico." << std::endl;
    return false;
  }
}
